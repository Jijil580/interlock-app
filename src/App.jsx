...
function WorkerReport({ user }) {
  ...
  const downloadReport = (r) => {
    ...
    a.href = url;
    a.download = `Site_Report_${r.workerName}_${r.startingDate}.txt`;
    a.click();
        ...
        <Modal title="Site Report" onClose={() => setModal(false)}>
          <div className="space-y-3">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 space-y-2">
              ...
              <Textarea label="Extra Work" value={form.extraWork} onChange={(e)=>setForm({...form,extraWork:e.target.value})} placeholder="Any extra work done..." />
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Input label="Total Working Area (sqft)" type="number" value={form.totalWorkingArea} onChange={(e)=>setForm({...form,totalWorkingArea:e.target.value})} />
                <Input label={`Total Amount (${CURRENCY})`} type="number" value={form.totalAmount} onChange={(e)=>setForm({...form,totalAmount:e.target.value})} />
              </div>
            </div>

            <Textarea label="📝 Note" value={form.note} onChange={(e)=>setForm({...form,note:e.target.value})} placeholder="Any notes..." />
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 space-y-2">
              ...
              <Input label="Amount Received By" value={form.amountReceivedBy} onChange={(e)=>setForm({...form,amountReceivedBy:e.target.value})} placeholder="Name of person" />
            </div>

            <Textarea label="🧱 Material Supply" value={form.materialSupply} onChange={(e)=>setForm({...form,materialSupply:e.target.value})} placeholder="Materials supplied..." />
      ...
      {viewModal && (
        <Modal title="Site Report Details" onClose={() => setViewModal(null)}>
          <div className="space-y-3">
            ...
            {viewModal.extraMaterial && <div className="bg-orange-50 border border-orange-200 rounded-xl p-3"><div className="text-xs font-bold text-orange-600 mb-1">🧱 Extra Material Using</div><div className="text-sm text-gray-700">{viewModal.extraMaterial}</div></div>}
                ...
                {["supervisor","office","admin"].map((role) => (
                  <div key={role} className={`rounded-xl border p-3 text-center ${viewModal.signatures?.[role]?"bg-green-50 border-green-300":"bg-white border-gray-200"}`}>
                    ...
                    {!viewModal.signatures?.[role] && (user.role === role || user.role === "admin") && (
                      <button onClick={() => signReport(viewModal._id, role)}
                        className="mt-2 bg-green-500 text-white px-2 py-1 rounded-lg text-xs font-bold hover:bg-green-600 w-full">
                        Sign
                      </button>
                    )}
